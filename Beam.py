...
with open("Problems/Exercise_01.txt", "r") as exercise:
    ...
    internal_forces_obj = list(filter(lambda x: x.get_support().get_name() != "Free", axis_points))
...
def create_segments(points):
    ...
    for i in range(len(points) - 1):
        segment = Segment(points[i], points[i + 1], points[i].get_distributed_force())
        segment.set_shear_function(segment.get_shear_function() + previous_shear_force)
        previous_shear_force = segment.shear_function_point_2()

        x = sp.symbols('x')
        segment.__bending_function = sp.integrate(segment.get_shear_function(),
                                                  x) + segment.get_point_1().get_bending_moments() + previous_bending_moment
...
def compute_shear_forces_sum(points, segments_y):
    ...
    for point_x in points:
        shear_forces_sum += point_x.get_concentrated_force()
    for segment in segments_y:
        shear_forces_sum += segment.get_distributed_force() * segment.get_length()
    return Eq(sum(x.get_ya() for x in points if x.get_ya() is not None), -1 * shear_forces_sum)
...
def compute_moments_sum(points, segments_m):
    ...
    internal_forces = list(filter(lambda x: x.get_support().get_name() != "Free", axis_points))
    for point_x in points:
        if point_x.get_support().get_name() != "Free":
            left_hand_side = point_x.get_ya() * (internal_forces[0].get_x() - point_x.get_x()) + point_x.get_ma()
        else:
            moments_sum += point_x.get_concentrated_force() * (internal_forces[0].get_x() - point_x.get_x()) + point_x.get_bending_moments()
    for segment in segments_m:
        if segment.get_distributed_force() != 0:
            moments_sum += segment.get_distributed_force() * segment.get_length() * (internal_forces[0].get_x() - segment.get_point_2().get_x()
...
def plot_shear_diagram(list_of_segments):
    ...
    for segment in list_of_segments:
        x_values.append(segment.get_point_1().get_x())
        y_values.append(float(segment.shear_function_point_1()))
        x_values.append(segment.get_point_2().get_x())
...
if integrity_check(axis_points):
    ...
    sol = solve((sum_y, sum_m), [x.get_ya() for x in internal_forces_obj] +
    ...
    for segment in segments:
        if segment.get_point_1().get_name().isalpha():
            reaction += segment.get_point_1().get_ya()
        segment.set_shear_function(segment.get_shear_function() + reaction)

        x = sp.symbols('x')
        segment.set_bending_function(sp.integrate(segment.get_shear_function(),
                                                  x) + segment.get_point_1().get_bending_moments() + previous_bending_moment)
        previous_bending_moment = segment.bending_moment_point_2()
